...
public class ArtworkCache {
    private static final String TAG = "DashWatch/ArtworkCache";
    ...
    public synchronized void maybeDownloadCurrentArtworkSync() {
        ...
        try {
            ...
            Log.e(TAG, "Error downloading current artwork. URI: " + currentArtwork.getImageUri(), e);
            if (e.isRetryable()) {
                // TODO: scheduleRetryArtworkDownload();
            }
            //EventBus.getDefault().postSticky(new ArtworkLoadingStateChangedEvent(false, true));
            return;
        }

        // TODO: cancelArtworkDownloadRetries();
        ...
        try {
            ...
            Log.e(TAG, "Error loading and caching current artwork.", e);
            destFile.delete();
            //EventBus.getDefault().postSticky(new ArtworkLoadingStateChangedEvent(false, true));
            // TODO: scheduleRetryArtworkDownload();
            return;
    ...
    public File getArtworkCacheFile(ComponentName source, Artwork artwork) {
        ...
        if (cacheRootForSource == null) {
            Log.w(TAG, "Empty source cache root.");
            return null;
        }

        if (artwork == null) {
            Log.w(TAG, "Empty artwork info.");
            return null;
        }

        Uri uri = artwork.getImageUri();
        if (uri == null) {
            Log.w(TAG, "Empty artwork image.");
            return null;
        }
