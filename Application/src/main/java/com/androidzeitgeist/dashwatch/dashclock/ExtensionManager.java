package com.androidzeitgeist.dashwatch.dashclock;
 ...
 *
 * This class is heavily based on the ExtensionManager class of DashClock.
 */
public class ExtensionManager {
    private static final String TAG = "DashWatch/ExtensionManager";
    ...
    private String createDefaultExtensionList() {
        ...
        // DashWatch: By default all world readable extensions are enabled
        for (ExtensionListing extension : getAvailableWorldReadableExtensions()) {
            if (sb.length() > 0) {
                sb.append(",");
            }
            sb.append(extension.componentName.flattenToString());
        }
    ...
    /**
     * Returns a listing of all available (installed) extensions that are world readable.
     */
    public List<ExtensionListing> getAvailableWorldReadableExtensions() {
        List<ExtensionListing> availableExteions = new ArrayList<ExtensionListing>();

        for (ExtensionListing extension : getAvailableExtensions()) {
            if (extension.worldReadable) {
                availableExteions.add(extension);
            } else {
                Log.e(TAG, "Extension not world readable: " + extension.componentName);
            }
        }

        return availableExteions;
    }
